...
## Tier 1: Absolute Must-Know (80% of your interview)

These appear in nearly every interview, regardless of level (junior to senior).

### 1. Arrays & Hashing (The King)

- **Why so frequent:** They are simple, fast to code, and test your ability to optimize space/time.
...
- **Why so frequent:** Real-world data is text. They test attention to detail (character encoding, immutability).
...
- **Why so frequent:** It shows if you can break complex problems into simpler sub-problems. Crucial for mid/senior roles.
- **Key patterns:** Fibonacci style (climbing stairs), 0/1 Knapsack (can you partition array into equal sums?), Grid paths (unique paths, robot in a grid), House Robber (max non-adjacent sum).
...
## Tier 2: Very Frequent (Get these for a passing grade)

### 4. Linked Lists

- **Why so frequent:** Tests your pointer manipulation skills. The problems are usually short to solve, perfect for a 15-minute live coding segment.
...
- **Why so frequent:** You rarely implement from scratch, but you MUST know when to use which sort and the complexity.
...
- **Why so frequent:** For more complex systems (social networks, maps, dependency resolution).
...
- **Bit Manipulation:** XOR tricks (find single number in pairs), bit masking. *Less common in general, but Google/Apple love it.*
- **Tries (Prefix Trees):** Word search, autocomplete, longest common prefix. *Good for "word" problems.*
- **Backtracking:** Generate all permutations/subsets, N-Queens. *Often a sign you need DP instead.*
...
- Low-level memory management (unless role is embedded/C++)

---

## Your 2-Week Study Strategy to *Remember* These

> **Don't just read. Active recall is key.**
...
| 5–6 | Linked Lists + Binary Trees (DFS & BFS) | Implement reversal and traversals from scratch without IDE |
| 7–8 | Binary Search + Sorting (custom comparators) | Practice rotated array search and custom sort rules |
| 9–10 | Stacks, Queues, Heaps (Top K problems) | Do "valid parentheses", "min stack", "Kth largest" |
| 11–12 | Graphs (BFS/DFS for Number of Islands) | Solve islands, shortest path, and cycle detection |
| 13–14 | **Mock interviews** | Use Pramp or a friend. Time yourself (25 min problem + 5 min discussion) |

---

## Final Pro Tip

For every problem you practice, **say out loud (or write)** the **Time Complexity** and **Space Complexity** before writing a single line of code. Interviewers care about this as much as the solution.
